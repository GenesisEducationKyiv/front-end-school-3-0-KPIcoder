...
export function GenreTagSelect({ value, onChange, error }: GenreTagSelectProps) {
  ...
  return (
    <FormItem>
      ...
      <Select onValueChange={handleSelectGenre} disabled={remainingGenres.length === 0}>
        <SelectTrigger>
          <SelectValue placeholder={remainingGenres.length ? 'Select a genre' : 'All available genres selected'} />
        </SelectTrigger>
        <SelectContent>
      ...
      <div className="flex flex-wrap gap-2 mt-2">
        {value.length > 0 ? (
          value.map((genre) => (
            <Tag key={genre} variant="default" onRemove={() => removeGenre(genre)} removeAriaLabel="Remove genre">
              {genre}
            </Tag>
          ))
        ) : (
