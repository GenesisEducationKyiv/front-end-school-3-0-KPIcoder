...
const TrackList = ({ tracks }: TrackListProps) => {
  ...
  const getConfirmationText = () => {
    const trackNames = tracks.filter((t) => selectedIds.has(t.id)).map((track) => `"${track.title}"`);
  ...
  return (
    <>
      ...
      <AudioPlayerModal
        open={!!playingTrackId}
        onOpenChange={() => setPlayingTrackId(null)}
        trackId={playingTrackId!}
      />

      <EditTrackForm isOpen={isEditDialogOpen} onClose={handleCloseEditDialog} track={editingTrack} />
